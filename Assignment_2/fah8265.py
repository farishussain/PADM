...
def get_next_state(state, action):
    if state in danger_states or state == goal_state:
        return state
    else:
        i, j = state
        if action == 'Right':
...
def get_transition_states(state, action):
    ...
    if action == 'Right':
        unintended = [get_next_state(state, 'Left'), get_next_state(state, 'Up'), get_next_state(state, 'Down')]
    elif action == 'Left':
        unintended = [get_next_state(state, 'Right'), get_next_state(state, 'Up'), get_next_state(state, 'Down')]
    elif action == 'Up':
        unintended = [get_next_state(state, 'Right'), get_next_state(state, 'Left'), get_next_state(state, 'Down')]
    elif action == 'Down':
        unintended = [get_next_state(state, 'Right'), get_next_state(state, 'Left'), get_next_state(state, 'Up')]
...
for k in range(1, 6):
    ...
    for state in states:
        ...
        V_new[state] = round(max(action_values), 2)  # Include the living reward for the current state
    V = V_new
    V_k.append(V.copy())
    print(f"V{k}:")
...
# Extract V4 values
V4 = {state: round(value, 2) for state, value in V_k[3].items()}

# Calculate Q5 values based on V4
Q = {}
for state in states:
    Q[state] = {}
    for action in actions:
        intended, unintended = get_transition_states(state, action)
        value = transition_prob['intended'] * (reward(intended) + gamma * V4[intended])
        for unintended_state in unintended:
            value += transition_prob['unintended'] * (reward(unintended_state) + gamma * V4[unintended_state])
        Q[state][action] = round(value, 2)

print("Q5 values based on V4:")
for state in states:
    print(f"State {state}:")
    for action in actions:
