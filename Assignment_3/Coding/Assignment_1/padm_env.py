...
class PadmEnv(gym.Env):
    ...
    The student must attend lectures (states), complete assignments, give a presentation, and give an exam while avoiding deregistration (blocker).
    """

    def __init__(self, grid_size=7, goal_coordinates=(6, 6), require_all_tasks_completed=False, num_tasks=0):
        """
        Initialize the environment.
        Parameters:
        grid_size (int): The size of the grid.
        goal_coordinates (tuple): The coordinates of the goal (exam).
        require_all_tasks_completed (bool): Whether all tasks must be completed before reaching the goal.
        ...
        self.require_all_tasks_completed = require_all_tasks_completed  # Condition for completing all tasks
        self.num_tasks = num_tasks  # Number of tasks to include
    ...
    def initialize_tasks(self):
        """
        Initialize tasks with their positions and rewards.
        Returns:
        ...
        return all_tasks[:self.num_tasks]
    ...
    def reset(self):
        ...
        self.tasks = self.initialize_tasks()  # Reset tasks
    ...
    def step(self, action):
        ...
        if any(np.array_equal(new_state, blk["position"]) for blk in self.blockers):
            ...
            if (0 <= new_state[0] < self.grid_size) and (0 <= new_state[1] < self.grid_size):
                ...
                # Check if student has reached the examination
                if np.array_equal(new_state, self.goal_state):
                    if self.require_all_tasks_completed:
                        if len(self.tasks) == 0:
                            reward += 50  # Bonus reward for reaching the goal after completing all tasks
                            done = True
                            info = {"course_completed": True, "days_to_exam": days_to_exam}
                        else:
                            reward -= 0  # Penalty for reaching the goal without completing all tasks
                            info = {"incomplete_tasks": True, "days_to_exam": days_to_exam}
                    else:
                        reward += 50  # Bonus reward for reaching the goal
                        done = True
                        info = {"course_completed": len(self.tasks) == 0, "days_to_exam": days_to_exam}
            else:
                # Student attempted to move out of bounds
                reward = 0  # Penalty for attempting to move out of bounds
                done = False
...
if __name__ == "__main__":
    env = PadmEnv(require_all_tasks_completed=False, num_tasks=0)  # Initialize with custom number of tasks
    state = env.reset()
    for _ in range(500):
...
# -----------
def create_env(goal_coordinates, hell_state_coordinates, require_all_tasks_completed, num_tasks):
    # Create the environment:
    # -----------------------
    env = PadmEnv(goal_coordinates=goal_coordinates, require_all_tasks_completed=require_all_tasks_completed, num_tasks=num_tasks)
